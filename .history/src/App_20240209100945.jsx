...
function App() {
  ...
  let item = ["Milk2", "Milk1", "Chocolate", "Mango", "Grapes", "Coco"];
  let itemList;
  let empty;
  useEffect(() => {
    ...
    if (searchWord != "" && variable == false) {
      for (let i = 0; i < item.length; i++) {
        ...
        console.log(searchWord == item[i]);
        for (let j = 0; j <= itemElement.length; j++) {
          console.log(itemElement[j]);
          let searchItem = [searchWord.split("").toString()];
          for (let k = 0; k <= searchItem.length; index++) {
            const element = array[index];
          }
          if (itemElement[j].includes(searchItem[j])) {
